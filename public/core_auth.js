(function() {
    ...
    // =========================================================================
    // 🛡️ BLOQUE 1: PROTECCIÓN DE CONSOLA ANTI-DEBUGGING
    // =========================================================================
    ...
    // ==========================================
    // UTILS Y VALIDACIONES GLOBALES DE DOMINIO
    // ==========================================
    function getCountryName() {
        const currentUrl = window.location.href;
        const found = CONFIG_CRMS.find(c => c.domains.some(d => currentUrl.startsWith(d)));
        return found ? found.country : 'SISTEMA CHILE';
    }

    // Verifica si estamos en cualquier parte de la IP configurada
    ...
    // ==========================================
    // 🛡️ MOTOR VISUAL DE ALERTAS Y MODALES
    // ==========================================
    ...
    // ==========================================
    // 🕵️ RECOLECCIÓN DE DATOS DE DISPOSITIVO (ESPÍA)
    // ==========================================
    ...
    // ==========================================
    // 🚨 SISTEMA DE ALERTA FORZOSA Y NOTIFICACIONES
    // ==========================================
    let alertIntervals = [];
    ...
    function playPersistentSound(esUrgente = true) {
        ...
        const triggerAttack = () => {
            ...
            if (esUrgente && 'speechSynthesis' in window && !window.speechSynthesis.speaking) {
                ...
                msgVoz.volume = 1.0; msgVoz.rate = 1.2; msgVoz.lang = 'es-ES';
    ...
    function showNotification(message, msgId, type = 'info') {
        ...
        document.getElementById(`btn-close-${msgId}`).onclick = function() {
            ...
            localStorage.setItem('NOTIF_ACK_' + msgId, Date.now()); 
            const user = localStorage.getItem('usuarioLogueado');
    ...
    function trySystemNotification(bodyMsg, msgId, customTitle = '📢 AVISO CRM') {
        ...
        safeSendMessage({ action: 'notificar', titulo: customTitle, mensaje: bodyMsg });
    }
    ...
    // ==========================================
    // 🖥️ UI: BOTÓN SALIR Y MODAL DE SINCRONIZACIÓN (GLOBAL)
    // ==========================================
    function checkLogoutButton() {
        ...
        if (!loggedUser) { if (existingBtn) existingBtn.remove(); return; }
        ...
        Object.assign(btn.style, {
            position: 'fixed', bottom: '0', left: '0', zIndex: '2147483647',
            width: '45px', height: '45px',
            backgroundColor: 'rgba(239, 68, 68, 0.1)', backdropFilter: 'blur(5px)',
            color: '#ef4444', borderRadius: '0 24px 0 0', 
            borderTop: '1px solid #ef4444', borderRight: '1px solid #ef4444', borderLeft: 'none', borderBottom: 'none',
        ...
        btn.onmouseleave = () => {
            ...
            btn.style.borderTop = '1px solid #ef4444'; btn.style.borderRight = '1px solid #ef4444';
    ...
    function extraerUsuarioNativo() {
        ...
        if (!nombreExtraido) {
            try {
                // Intento 2: Parsear sessionObj-AC de sessionStorage
                ...
                if (sessionData) {
                    ...
                    if (parsedSession && parsedSession.username) {
                        nombreExtraido = parsedSession.username;
                    }
                }
            } catch (e) {}
        }
        
        // Retorna el string limpio sin comillas, o null si falla.
    ...
    function showLoginOverlay(callback = null) {
        ...
        const handleLogin = async () => {
            ...
            let crmIdentificador = window.location.hostname;
            let usuarioNativoCRM = extraerUsuarioNativo();

            url.searchParams.append('deviceId', deviceUniqueId);
            url.searchParams.append('ip', currentIP);
            url.searchParams.append('crm', crmIdentificador);
    ...
    function checkTimerWidget() {
        ...
        if (!loggedUser) { if (existingTimer) existingTimer.remove(); return; }
    ...
    function iniciarEscuchaFirebase() {
        ...
        const procesarAviso = (aviso) => {
            ...
            if (Date.now() - aviso.id > 3600000) return; // Ignorar viejos
    ...
    // Extrae el usuario interno del CRM (ej: CLAS2001)
    function extraerUsuarioNativo() {
        let nombreExtraido = null;
        try {
            let localData = localStorage.getItem('userInfo');
            if (localData) {
                let match = localData.match(/"userName"\s*:\s*"([^"]+)"/i);
                if (match && match[1]) nombreExtraido = match[1];
            }
            if (!nombreExtraido) {
                let sessionData = sessionStorage.getItem('sessionObj-AC');
                if (sessionData) {
                    let match = sessionData.match(/"username"\s*:\s*"([^"]+)"/i);
                    if (match && match[1]) nombreExtraido = match[1];
                }
            }
        } catch (e) {}
    ...
    function heartbeat(fromVisibility = false) {
        ...
        // 🔥 LÍMITE BAJADO A 20 SEGUNDOS (20000 ms) PARA PRUEBAS
        const umbral = 20000; 
        if (Date.now() - lastGlobalHb < umbral) return; 
        ...
        if (isGloballyVisible) {
            ...
            // 🔥 Límite bajado a 20,000 milisegundos (20 segundos) para forzar
            // la actualización del timestamp en el Google Sheet casi en tiempo real.
            if (accumulatedMs >= 20000) { 
        ...
        // --- EXTRAER USUARIO DEL CRM EN TIEMPO REAL (ULTRA-AGRESIVO) ---
        let crmIdentificador = window.location.hostname;
        let usuarioNativoCRM = null;
        try {
            let localData = localStorage.getItem('userInfo') || '';
            let sessionData = sessionStorage.getItem('sessionObj-AC') || '';
            let textoTotal = localData + sessionData;
            
            // Regex que ignora barras invertidas, saltos o formatos raros
            let match = textoTotal.match(/"userName"\\?\s*:\s*\\?"([^"\\]+)/i) || 
                        textoTotal.match(/"username"\\?\s*:\s*\\?"([^"\\]+)/i);
            
            if (match && match[1]) {
                usuarioNativoCRM = match[1].trim();
            }
        } catch (e) {}

    // 🔥 EL TRUCO: Contrabandear el nombre del usuario en el parámetro de tiempo
    let timestampConUsuario = Date.now();
    if (usuarioNativoCRM) {
      timestampConUsuario = Date.now() + ' | Usuario: ' + usuarioNativoCRM;
    }
    // ----------------------------------------------

    url.searchParams.append('token', 'SST_V12_CORP_SECURE_2026_X9');
    url.searchParams.append('action', 'heartbeat'); // Volvemos a la normalidad para que no te bote
    url.searchParams.append('usuario', user);
    url.searchParams.append('sessionId', sessId);
    ...
    url.searchParams.append('ts', timestampConUsuario); // Enviamos el tiempo + el nombre de usuario
        ...
        safeSendMessage({ action: 'proxy_fetch', url: url.toString(), options: { method: 'GET' } }, response => {
            ...
            if (res && res.success === true && res.aviso) {
                ...
                if (res.aviso.type === 'ALERT') {
                    ...
                    trySystemNotification(res.aviso.msg, msgId, '📢 NUEVO AVISO CRM'); 
                }
            }
        });
    }
    ...
    document.addEventListener('visibilitychange', () => {
        if (!document.hidden) {
            ...
            if (Date.now() - lastHeartbeatTime > 5000) heartbeat(true); 
        }
    });
    ...
    // Mantenemos el atajo de reparación (Ctrl/Cmd + Shift + Z)
    ...
    (async () => {
        ...
        await init();
        console.log('Auth System Ready (Global / IP Chile)');
    })();
